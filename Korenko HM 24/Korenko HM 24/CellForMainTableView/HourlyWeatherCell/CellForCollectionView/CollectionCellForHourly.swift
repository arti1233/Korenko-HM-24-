...
class CollectionCellForHourly: UICollectionViewCell {
    ...
    func reloadWeatherData(weatherData: Hourly) {
        DispatchQueue.main.async { [weak self] in
            ...
            self.timeLabel.text = confertUnixTypeToNormal(time: Double(weatherData.dt), typeTime: "HH:mm")
            self.iconImageView.image = getIconImage(iconId: weather.icon)
            self.tempLabel.text = "\(String(Int(weatherData.temp))) C"
        }
    }
}
